...
Laplace='gL'                        # 'LB': LB-operator;  'gL': graph Laplacian
params['algo']      = Laplace
...
# ----- Hyper-parameters ------------------------------------------------------
params['num_epochs']     = 25       # number of epochs
...
data_trainvalid = np.array(f.get('data_train')).astype(float)    
data_test = np.array(f.get('data_test')).astype(float)
target_trainvalid = np.transpose(np.array(f.get('label_train')).astype(int))[0]
target_test = np.transpose(np.array(f.get('label_test')).astype(int))[0]
protected_trainvalid = np.transpose(np.array(f.get('site_train')).astype(int))[0]
p_test = np.transpose(np.array(f.get('site_test')).astype(int))[0]
...
protected_trainvalid = np.transpose(np.vstack([np.transpose(1-protected_trainvalid), np.transpose(protected_trainvalid)]))

data_train, target_train, p_train, data_valid, target_valid, p_valid = hf.train_valid_data(data_trainvalid, target_trainvalid, protected_trainvalid, valid_ratio)
...
print('P Group ratio in train set: ',   len(p_train)-sum(p_train),':',sum(p_train),'\n')
print('P Group ratio in Valid set: ',   len(p_valid)-sum(p_valid),':',sum(p_valid),'\n')
...
val_loss, val_accuracy, val_fscore, val_sensitivity, val_specificity, val_precision, val_ppv, val_npv, val_gmean, val_acc_diff, val_prec_diff \
= model.fit(data_train, target_train, p_train, data_valid, target_valid, p_valid, params['learning_rate'],params['num_epochs'],params['decay_steps'])
