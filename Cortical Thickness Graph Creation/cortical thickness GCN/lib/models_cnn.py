...
configs = {}
configs['NUM_INNER'] = 1
configs['ETA_INIT'] = 2.0
configs['ETA_BETA'] = 1.0001
configs['LAM0_PRIOR'] = 0.
configs['LAM1_PRIOR'] = 0.
configs['LR'] = 0.1
...
class base_model(object):
    ...
    def predict(self, data, labels=None, p_labels=None, sess=None):
        loss = 0
        size = data.shape[0]
        ...
        for begin in range(0, size, self.batch_size):
            ...
            feed_dict = {self.ph_data: batch_data, self.ph_dropout: 1, self.ph_eta: configs['ETA_INIT'],self.ph_lam0: configs['LAM0_PRIOR'], self.ph_lam1: configs['LAM1_PRIOR']}
            
            # Compute loss if labels are given.
            if labels is not None:
                if labels.ndim == 1:
                    batch_labels = np.zeros(self.batch_size)
                    batch_p_labels = np.zeros(self.batch_size)
                    
                    batch_labels[:end-begin] = labels[begin:end]
                    batch_p_labels[:end-begin] = p_labels[begin:end]

                    feed_dict[self.ph_labels] = batch_labels
                    feed_dict[self.ph_p] = batch_labels
...
#                    batch_pred, batch_loss = sess.run([self.op_prediction, self.op_loss], feed_dict)
                    ...
                    batch_p_labels = np.zeros((self.batch_size, p_labels.shape[1]))
                    batch_p_labels[:end-begin,:] = p_labels[begin:end,:]
                    feed_dict[self.ph_p] = batch_p_labels
#                    batch_pred, batch_loss = sess.run([self.op_prediction, self.op_loss], feed_dict)
                    batch_pred, batch_loss, batch_logits = sess.run([self.op_prediction, self.op_loss, self.op_logits], feed_dict) # output logits #20200912     
...
#            return predictions, loss * self.batch_size / size       
    ...
    def evaluate(self, data, labels, p_labels, sess=None, training_phase=False):
        """
...
#        predictions, loss = self.predict(data, labels, sess)
        predictions, loss, logits = self.predict(data, labels, p_labels, sess)     
        if labels.ndim > 1:
            labels = np.copy(labels[:,1])
            p_labels = np.copy(p_labels[:,1])
        ...
        if not training_phase:
            ...
            accuracy_c1, f_na, s_na, sp_na, precision_c1, ppv_na, npv_na, gmean_na = self.top_k_error(labels[c_1], predictions[c_1], prevalence, 1)

            accuracy_c0, f_na, s_na, sp_na, precision_c0, ppv_na, npv_na, gmean_na = self.top_k_error(labels[c_0], predictions[c_0],prevalence, 1)

            accuracy_diff = 100*(accuracy_c0-accuracy_c1)
            precision_diff = 100*(precision_c0-precision_c1)
...
#       
        ...
        return string, loss, accuracy, f1, sensitivity, specificity, precision, ppv, npv, gmean, predictions, logits, accuracy_diff, precision_diff  


    def fit(self, train_data, train_labels, p_labels, val_data, val_labels, p_valid, learning_rate, num_epochs, decay_steps, finetune=False, finetune_fc=False):
...
#        
        ...
        self.state = {'eta': configs['ETA_INIT'], 'lam0': configs['LAM0_PRIOR'], 'lam1': configs['LAM1_PRIOR']}
        ...
        for step in range(1, num_steps+1):
            ...
            if self.random_batch_sampling_train==False:
                ...
                batch_data, batch_labels, batch_p = train_data[idx,:], train_labels[idx], p_labels[idx]
            
            if type(batch_data) is not np.ndarray:
                batch_data = batch_data.toarray()  # convert sparse matrices
                
            feed_dict = {self.ph_data: batch_data, self.ph_labels: batch_labels, self.ph_p: batch_p, self.ph_dropout: self.dropout, self.ph_lr: self.learning_rate, self.ph_eta: configs['ETA_INIT'],self.ph_lam0: configs['LAM0_PRIOR'], self.ph_lam1: configs['LAM1_PRIOR']} # new learning rate for pretraining
...
#                string, loss_train, acc_train, fscore_train, sensitivity_train, specificity_train, precision_train, ppv_train, npv_train, gmean_train, predictions = self.evaluate(train_data, train_labels, sess)            
                string, loss_train, acc_train, fscore_train, sensitivity_train, specificity_train, precision_train, ppv_train, npv_train, gmean_train, predictions,logits = self.evaluate(train_data, train_labels, p_labels, sess, training_phase=True)    #20200912
...
#                string, loss_valid, acc_valid, fscore_valid, sensitivity_valid, specificity_valid, precision_valid, ppv_valid, npv_valid, gmean_valid, predictions = self.evaluate(val_data, val_labels, sess)   
                string, loss_valid, acc_valid, fscore_valid, sensitivity_valid, specificity_valid, precision_valid, ppv_valid, npv_valid, gmean_valid, predictions,logits, acc_diff_val, prec_diff_val = self.evaluate(val_data, val_labels, p_valid, sess)   #20200912
                val_loss_list.append(loss_valid)
                val_accuracy_list.append(acc_valid)
                val_acc_diff_list.append(acc_diff_val)
                val_prec_diff_list.append(prec_diff_val)
                ...
                valid_summ.value.add(tag='valid_acc_diff', simple_value=acc_diff_val)
                valid_summ.value.add(tag='valid_prec_diff', simple_value=prec_diff_val)
                ...
                print('Validation top1 acc diff = %.4f' % acc_diff_val)
                print('Validation top1 prec diff = %.4f' % prec_diff_val)
                print('----------------------------')
                ...
                df = pd.DataFrame(data={'step':step_list, 'train_accuracy':train_accuracy_list, 'train_fscore':train_fscore_list, 'train_sensitivity':train_sensitivity_list, 
                      ...
                      'validation_precision':val_precision_list, 'validation_ppv':val_ppv_list, 'validation_npv':val_npv_list, 'validation_gmean':val_gmean_list, 'acc_diff_val': acc_diff_val, 'prec_diff_val': prec_diff_val })
...
#            string, loss_valid, acc_valid, fscore_valid, sensitivity_valid, specificity_valid, precision_valid, ppv_valid, npv_valid, gmean_valid, predictions = self.evaluate(val_data, val_labels, sess)
        ...
        return val_loss_list, val_accuracy_list, val_fscore_list, val_sensitivity_list, val_specificity_list, val_precision_list, val_ppv_list, val_npv_list, val_gmean_list, val_acc_diff_list, val_prec_diff_list
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('inputs'):
                ...
                if C == 1:
                    ...
                    self.ph_p = tf.placeholder(tf.int32, (self.batch_size), 'p_labels')
                else:    
                    self.ph_labels = tf.placeholder(tf.int32, (self.batch_size, C), 'labels')
                    self.ph_p = tf.placeholder(tf.int32, (self.batch_size, C), 'p_labels')

                self.ph_dropout = tf.placeholder(tf.float32, (), 'dropout')
                self.ph_lr = tf.placeholder(tf.float32, (), 'learning_rate')
                self.ph_lam0 = tf.placeholder(tf.float32, (), 'lam0')
                self.ph_lam1 = tf.placeholder(tf.float32, (), 'lam1')
                self.ph_eta = tf.placeholder(tf.float32, (), 'eta')
            ...
            self.op_loss, self.op_loss_average, self.state = self.loss(op_logits, self.ph_labels, self.ph_p, self.regularization, self.state)
...
#        self.decay_rate, self.decay_steps, self.momentum = decay_rate, decay_steps, momentum
        ...
        self.state = state = {'eta': configs['ETA_INIT'], 'lam0': configs['LAM0_PRIOR'], 'lam1': configs['LAM1_PRIOR']}
