...
def train_valid_data(input_data, input_target, input_p, valid_ratio):
    '''
...
#    patient_valid_index = random.sample(patient_index_all, int(math.floor(valid_ratio*num_patient)))
    ...
    p_valid = input_p[data_valid_index]
    ...
    p_train = input_p[data_train_index]
    
#    print('Number of control: ', num_control)
#    print('Number of patient: ', num_patient)
                     
    return data_train, target_train, p_train, data_valid, target_valid,p_valid # early prediction                   


def generate_train_batch(train_data, train_labels, p_labels, batch_size, patient_ratio, random_batch_sampling_train=False):
    '''
    ...
    if random_batch_sampling_train is True:
        ...
        batch_p = p_labels[data_train_index]

    return batch_data, batch_label, batch_p


def save_to_csv(idx, acc, acc_diff, prec_diff, fscore, sensitivity, specificity, precision, ppv, npv, gmean, file_name, wholetime, epochtime, mode='w', header=False):
  df = {'aaidx':idx, 'accuracy':acc, "acc_diff":acc_diff, "prec_diff":prec_diff, 'fscore':fscore, 'sensitivity':sensitivity, 'specificity':specificity, 
...
def print_classification_performance(prevalence, loss, acc, fscore, sensitivity, specificity, precision, ppv, npv, gmean, acc_diff, prec_diff):
  print( '----------------------------')
#  print( 'Classification performance for %s: ' % dataset)
  ...
  print('Validation top1 acc diff = %.4f' % acc_diff)
  print('Validation top1 prec diff = %.4f' % prec_diff)
  print( '----------------------------')
